...
class Thumbnail{
	...
	var $setTypeImg=array(
		...
		"bmp"=>array("image/bmp","ImageCreateFromBmp")
	);
	...
	function MakeNew(){
		...
		$this->blanc= imagecolorallocate ($this->image,0xff,0x00,0x00);
		imagealphablending($this->imageP, false);
		imagesavealpha($this->imageP, true);  
	}
	...
	function FinirPImagePNG(){
		imagecopyresampled($this->imageP, $this->image, 0, 0, 0, 0, $this->infoImage[2], $this->infoImage[3], $this->infoImage[0], $this->infoImage[1]);
		if($this->Text)
			imagestring ($this->imageP, 3, 2, $this->infoImage[3]-16, $this->Text, $this->blanc);
		imagepng($this->imageP, $this->filename2);
		imagedestroy($this->imageP);
	}
}
?>