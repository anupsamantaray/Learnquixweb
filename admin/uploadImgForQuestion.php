...
if($img_type=='image/png' || $img_type=='image/jpeg' || $img_type=='image/gif' || $img_type=='image/bmp'){
	...
	if(move_uploaded_file($tmp_name,$path)){
		 ...
		 $httphost = $_SERVER['HTTP_ORIGIN'].'/admin/';
		 if($_SERVER['HTTP_ORIGIN'] == 'http://localhost'){
			 $httphost = $_SERVER['HTTP_ORIGIN'].'/Learnquixweb/admin/';
		 }
		 die(json_encode(array("status"=>"1","server"=>$httphost,"img_path"=>$httphost.$pic->filename2)));
	}
}
